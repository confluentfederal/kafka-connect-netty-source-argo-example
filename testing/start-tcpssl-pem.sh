...
curl -X POST -H "Content-Type: application/json" -H "Accept: application/json" -d @- http://localhost:8083/connectors << EOF
{
  ...
  "config": {
    ...
    "netty.ssl.certfile.location": "/opt/kafka-connect-plugins/certs/connect-combined.crt",
    "netty.ssl.keyfile.location": "/opt/kafka-connect-plugins/certs/connect.key",
    "netty.ssl.keystore.type": "PEM"
  }
}
EOF
